...
import type { DepNode, DepEdge, Severity } from '@/types';
import { getFileContent, getRepoTree } from '@/lib/github';
import { PACKAGE_NAME, createLimiter, fetchPackument, checkTyposquat, queryOSV } from '@/lib/npm/registry';
import { analyzeVersion, youngDependencySignal, type Packument } from './deprisk';
...
async function flagTyposquats(nodes: DepNode[]): Promise<void> {
  await Promise.allSettled(nodes.filter((n) => n.isDirect).map(async (node) => {
    const signal = await checkTyposquat(node.name);
    if (signal) node.signals = [...(node.signals ?? []), signal];
  }));
}
